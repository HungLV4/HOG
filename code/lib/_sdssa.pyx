...
cdef float sdssa(int[:, ::1] data,
				int column_ind, int row_ind,
				int size_column, int size_row,
				int size_window_column, int size_window_row,
				float[:, ::1] integral,
				float[:, ::1] integral_sqr) nogil:
	
	cdef float S1, S2
	cdef float rxd = 0.0
	
	cdef int size_window_pixels = size_window_row * size_window_column
...
def sdssa_calc(int[:, ::1] data,
				int size_column, int size_row,
				int size_window_column, int size_window_row,
				float[:, ::1] integral,
				float[:, ::1] integral_sqr,
				float[:, :] output):
	cdef int x, y
